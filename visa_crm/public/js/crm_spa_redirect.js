...
(function () {
  ...
  // 1. DEFENSIVE WINDOW.OPEN GUARD: Intercept any attempt to open web.whatsapp.com in external tab
  if (typeof window !== 'undefined' && !window.__wa_open_guard_installed) {
  ...
  // 2. CAPTURE-PHASE EVENT LISTENER: Intercepts clicks before native Vue / DOM handlers execute
  if (typeof document !== 'undefined' && !document.__wa_click_listener_installed) {
    document.__wa_click_listener_installed = true;
    document.addEventListener(
      'click',
      function (e) {
        const target = e.target.closest('button, a, [role="button"], li, div');
        if (!target) return;
      ...
      true // capture: true guarantees execution before bubbling handlers
    );
  }
  ...
  function checkAndEnhanceSidebar() {
    ...
    // If no WhatsApp button exists in sidebar at all, inject our styled button after Call Logs
    if (!document.getElementById('crm-sidebar-whatsapp-launcher')) {
      let targetNav = null;
      ...
      if (targetNav && callLogsBtn) {
        ...
        if (!hasWA) {
          ...
          waBtn.className =
            'flex h-7.5 cursor-pointer items-center rounded text-ink-gray-8 duration-300 ease-in-out hover:bg-surface-gray-2 focus:outline-none focus:transition-none focus-visible:rounded focus-visible:ring-2 focus-visible:ring-outline-gray-3 mx-2 my-[1.5px]';
  ...
  // Monitor DOM changes for dynamic route views
  if (typeof document !== 'undefined') {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', checkAndEnhanceSidebar);
    } else {
      checkAndEnhanceSidebar();
    }

    const observer = new MutationObserver(checkAndEnhanceSidebar);
