...
PRINTS={"Visa Application":("Visa Application","<h2>Visa Application</h2><p>{{ doc.name }}</p>"),"Customer Profile":("Customer","<h2>Customer Profile</h2><p>{{ doc.customer_name or doc.name }}</p>"),"Payment Receipt":("Payment Schedule","<h2>Payment Receipt</h2><p>{{ doc.get_formatted('amount') if doc.meta.has_field('amount') else doc.name }}</p>"),"Document Checklist":("Customer Documents","<h2>Document Checklist</h2><p>{{ doc.document_type or doc.document_name or doc.name }}</p>")}
