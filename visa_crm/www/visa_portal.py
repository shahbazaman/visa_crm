...
def _applications(customer):
    ...
    return frappe.get_all("Visa Application",filters={"customer":customer} if has_field("Visa Application","customer") else {},fields=fields,order_by="modified desc")
...
def _documents(customer):
    ...
    return frappe.get_all("Customer Documents",filters={"customer":customer} if has_field("Customer Documents","customer") else {},fields=fields,order_by="modified desc")
...
def _payments(customer):
    ...
    return frappe.get_all("Payment Schedule",filters={"customer":customer} if has_field("Payment Schedule","customer") else {},fields=fields,order_by="due_date asc")
...
def _timeline(customer):
    ...
    return frappe.get_all("Communication Event",filters={"customer":customer} if has_field("Communication Event","customer") else {},fields=fields,order_by="event_datetime desc, modified desc",limit_page_length=30)
