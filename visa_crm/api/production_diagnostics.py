...
def queue_diagnostics(limit=100):
    ...
    for row in rows:
        ...
        out.append({"name":row.name,"current_stage":frappe.db.get_value("Lead Intake Queue",row.name,"current_stage") if has_field("Lead Intake Queue","current_stage") else row.status,"orchestration_status":frappe.db.get_value("Lead Intake Queue",row.name,"orchestration_status") if has_field("Lead Intake Queue","orchestration_status") else row.status,"retry_count":sum(stage.attempt_count or 0 for stage in stages) if stages else row.get("retry_count") or 0,"last_api_response":_clip(row.get("graph_api_response") or row.get("graph_payload")),"graph_api_request":_clip(row.get("graph_api_request")) or row.get("source_lead_id"),"scheduler_timestamp":row.get("processing_started_at") or row.modified,"processing_duration":sum(stage.duration_ms or 0 for stage in stages)/1000 if stages else round((done-started).total_seconds(),2) if started and done else None,"failure_reason":failed[0].last_error if failed else row.get("last_error"),"failed_stage":failed[0].stage if failed else None,"stages":stages,"modified":row.modified})
