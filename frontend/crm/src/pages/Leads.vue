<template>
  <LayoutHeader>
    ...
    <template #right-header>
      <Button
        v-if="route.params.viewType == 'list'"
        :variant="isTreeView ? 'solid' : 'subtle'"
        :iconLeft="isTreeView ? 'layers' : 'list'"
        :label="isTreeView ? __('Tree View') : __('List View')"
  ...
  <LeadsTreeView
    v-else-if="route.params.viewType == 'list' && isTreeView"
    :list="leads"
  />
  <LeadsListView
...
import LeadsTreeView from '@/components/ListViews/LeadsTreeView.vue'
...
const isTreeView = useStorage('crm_leads_tree_view', true)
